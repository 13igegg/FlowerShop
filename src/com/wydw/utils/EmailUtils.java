...
public class EmailUtils {
	...
	private static MimeMessage createMsg(Session session, String myAccount, User user) {
		...
		try {
			//3.1发件人 from
			message.setFrom(new InternetAddress(myAccount, "FlowerShop", "utf-8"));
			...
			//3.3生成邮件主题
			message.setSubject("FlowerShop账号激活邮件","utf-8");
			String ip = Inet4Address.getLocalHost().getHostAddress();
			String url = "http://"+ip+":8080/user?method=active&c="+Base64Utils.encode(user.getCode());
			//设置邮件正文 setContent 可以使用html标签
			message.setContent(user.getUsername()+",你好<br>欢迎注册FlowerShop! 请点击链接进行激活:<a href='"+url+"'>点击此处</a>","text/html;charset=utf-8");
			//设置邮件的发送时间 是立即发送
