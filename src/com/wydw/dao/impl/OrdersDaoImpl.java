package com.wydw.dao.impl;

import com.wydw.dao.OrdersDao;
import com.wydw.entity.Address;
import com.wydw.entity.Item;
import com.wydw.entity.Orders;
import com.wydw.entity.Product;
import com.wydw.utils.C3P0Utils;
import com.wydw.utils.StrUtils;
...
public class OrdersDaoImpl implements OrdersDao {
    ...
    public List<Orders> selectOrdersByUid(int uid) throws SQLException, InvocationTargetException, IllegalAccessException {
        ...
        Map<String,Object> oamp = new LinkedHashMap<>();
        for (Map<String, Object> map : list) {
            Set<String> smap = map.keySet();
            for (String s: smap) {

                String sk= StrUtils.removeCharAt(s,1);
                oamp.put(sk,map.get(s));

            }
            Orders orders = new Orders();
            Address address = new Address();

            BeanUtils.populate(orders, oamp);
            BeanUtils.populate(address, oamp);
    ...
    public Orders selectOrdersByOid(String oid) throws InvocationTargetException, IllegalAccessException, SQLException {
        ...
        Set<String> smap = map.keySet();
        Map<String,Object> namp = new LinkedHashMap<>();
        for (String s: smap) {

            String sk=StrUtils.removeCharAt(s,1);
            namp.put(sk,map.get(s));

        }
        Orders orders = new Orders();
        Address address = new Address();

        BeanUtils.populate(orders, namp);
        BeanUtils.populate(address, namp);
        orders.setAddress(address);
    ...
    public List<Item> selectItemsByOid(String oid) throws SQLException, InvocationTargetException, IllegalAccessException {
       ...
       String sql = "select p.p_id as pid, p.t_id as tid,p.p_name as pname,p.p_time as ptime ," +
        ...
        for (Map<String, Object> map : list) {
            ...
            BeanUtils.populate(product, namp);
            BeanUtils.populate(item, namp);
